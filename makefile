...
# nvcc -I ..\opencv\build\include -L ..\opencv\build\x64\vc15\lib main.cu network.cu layers.cu pooling_layer.cu input_layer.cu  -lcudnn -lcublas -o test -lopencv_world420
...
network.o: $(home)/network.cu $(home)/layers.h $(home)/input_layer.h $(home)/pooling_layer.h
	nvcc -c -arch=sm_35 -std=c++11 $(home)/network.cu


layers.o: $(home)/layers.cu $(home)/layers.h $(home)/input_layer.h $(home)/pooling_layer.h
	nvcc -c -arch=sm_35 -std=c++11 $(home)/layers.cu $(home)/input_layer.cu $(home)/pooling_layer.cu
