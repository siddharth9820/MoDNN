...
void offload_when_needed(DataLoader * dataloader,Dataset * dataset,seqNetwork * nn, vmm * mem_manager, int epochs)
{
  ...
  for(int epoch_no=0;epoch_no<epochs;epoch_no++)
  {
    ...
    for(int batch_no=0;batch_no<num_iters_in_epoch;batch_no++)
    {
      ...
      {
        //forward
        nn->allocate_mem_layer_fw2(0,mem_manager);
        //mem_manager->printNodes();
        for(int i=0;i<nn->num_layers;i++)
        {
            ...
            if(i==0)//enqueue_batch
            {
              ...
              nn->allocate_mem_layer_fw2(i+1,mem_manager);
            }
            else//forward pass
            {
        ...
        //   nn->allocate_mem_layer_bw2(i,mem_manager);
        //
        //   //mem_manager->printNodes();
        ...
        // }
        mem_manager->reset();
        while(!nn->locally_allocated_layers.empty())nn->locally_allocated_layers.pop();
        while(!nn->globally_allocated_layers.empty())nn->globally_allocated_layers.pop();
